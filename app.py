...
def build_player_season_stats(element, position):
    """Only the non-zero stats relevant to the player's position, in display order -
    mirrors how the Players screen's mockup shows a short, position-appropriate list
    rather than every stat FPL tracks for every player."""
    stats = [{"label": "Minutes", "value": element["minutes"]}]

    if position == "GKP":
        if element.get("saves"):
            stats.append({"label": "Saves", "value": element["saves"]})
        if element.get("clean_sheets"):
            stats.append({"label": "Clean sheets", "value": element["clean_sheets"]})
    else:
        if element.get("goals_scored"):
            stats.append({"label": "Goals", "value": element["goals_scored"]})
        if element.get("assists"):
            stats.append({"label": "Assists", "value": element["assists"]})
        if position == "DEF" and element.get("clean_sheets"):
            stats.append({"label": "Clean sheets", "value": element["clean_sheets"]})

    if element.get("bonus"):
        stats.append({"label": "Bonus", "value": element["bonus"]})
...
def get_players():
    ...
    for element in bootstrap["elements"]:
        # Skip anyone who hasn't played a minute this season - mostly untracked
        # squad players who'd just be noise in a search/browse list.
        if element.get("minutes", 0) <= 0:
            continue
        position = POSITION_TYPE_NAMES.get(element["element_type"], "UNK")
        players.append({
            "name": f'{element["first_name"]} {element["second_name"]}',
            "club": team_names.get(element["team"], "—"),
            "position": position,
            "points": element["total_points"],
            "stats": build_player_season_stats(element, position),
        })
...
def handler(event, context):
    ...
    try:
        ...
        if path.rstrip("/").endswith("/league/players"):
            return _response(200, get_players())
